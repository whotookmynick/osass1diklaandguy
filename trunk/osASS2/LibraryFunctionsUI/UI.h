...
//	OSUI(SystemCalls *systemCallsCaller, vector<int> *fdTable, int pid, int fatherPid, string pwd, map<int,OSUI*> *processTable);
	OSUI(SystemCalls *systemCallsCaller, vector<int> *fdTable, int pid, int fatherPid, map<int,OSUI*> *processTable);
	virtual ~OSUI();
	void run();
	/*
	 ...
	 * Make sure to also switch to the new process' int fd table and working directory.
	 */
