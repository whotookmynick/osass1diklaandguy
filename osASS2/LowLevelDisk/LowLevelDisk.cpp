...
void LowLevelDisk::initSuperBlock(int dataBlockSize,int numberOfInodes,int diskSize){
        ...
        _superBlock->rootInode=ROOT_INODE;//offset 2 in file
            writeDataToHardDisk(INODE_TABLE_BLOCK_NUM,(void*)&_superBlock->rootInode,OFFSET_SIZE_IN_BYTES);
...
//    LOG_DEBUG("init freeInodeList\n");
    ...
    int numOfOffsetInBlock = _superBlock->blockSize/4;
...
//    _superBlock->firstBlockOfFreeBlocksOffset = ((_superBlock->numOfBlocks) - numOfFreeBlocks)*blockSize;
    ...
    int numOfFreeInodeBlocks = (_superBlock->numOfInodes)/(_superBlock->blockSize)+1;
    int firstEmptyBlockNumber =(_superBlock->numOfBlocks) -_numOfBlocksInInodeTable - numOfFreeInodeBlocks;
    ...
    int numOfOffsetInBlock = blockSize/4;
    //int checkNewOffset = (_superBlock->lastFreeInode/_superBlock->blockSize)+1;//freeOffset
    while(numOfFreeBlocksToWrite>0){//(blockSize-i-1)//while there are blocks to store in freeBlockList
        if((i==numOfOffsetInBlock-1)and(numOfBlockInFreeBlockList>1)){//if we get the one befor last offset in block and there is more then one freeBlock to add
            //insert the next block of free blocks
            writeDataToHardDisk(freeOffset,(void*)&nextFreeBlock,OFFSET_SIZE_IN_BYTES);//freeOffset
            //jump to next block and fill it
            freeOffset=nextFreeBlock*blockSize;//freeOffset
...
LowLevelDisk::LowLevelDisk(int dataBlockSize,int numberOfInodes,int diskSize):_iNodeTable()
{
    ...
    if (existsFileSystem()){
            ...
            initSuperBlockFromHardDisk();
             _freeBlockesList = new FreeBlockList(_fd,_superBlock->firstEmptyBlock//firstBlockOfFreeBlocksOffset
                        ,_superBlock->firstEmptyBlock,_superBlock->lastEmptyBlock,*this);
             _freeInodesList=new FreeInodeList(_fd,_superBlock->firstFreeInode//firstBlockOfFreeInodesOffset
                     ,_superBlock->firstFreeInode, _superBlock->lastFreeInode,*this);
             _iNodeTable = new InodeList(_fd, _superBlock->inodeTableOffset, *this);
...
int  LowLevelDisk::allocateInode(){
    ...
    cout<<"  allocateInode  "<< node_id<<endl;
//    rmvNodeFromFreeNode();//remove the node frome free node list
    _iNodeTable->setActive(node_id,true);//init node details to defult
...
//    cout<<"setInodeType-  need to finish implement lists "<<endl;
    ...
    //    throw invalid_argument("no such file type");//TODO: add exception
    	...
    	this->_iNodeTable->setFileType(i_node,filetype);
    }
    pthread_mutex_unlock(&_RecMutex);
}
...
//    cout<<" getDataBlock--  need to finish implement lists  "<<endl;
    pthread_mutex_lock(&_RecMutex);
    if ((i_node<0) |(i_node>=(_superBlock->numOfInodes)) ){ //(dblock>=0) | (dblock<=9) |
        pthread_mutex_unlock(&_RecMutex);
